...
import { setTelemetryEnabled } from "@8medusa/telemetry"
...
function buildLocalCommands(cli, isLocalProject) {
  ...
  function resolveLocalCommand(command) {
    ...
    try {
      const cmdPath = resolveCwd.silent(`@8medusa/medusa/commands/${command}`)!
...
function isLocalMedusaProject() {
  ...
  try {
    ...
    inMedusaProject = !!(
      (dependencies && dependencies["@8medusa/medusa"]) ||
